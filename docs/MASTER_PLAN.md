...
**Status:** 🚧 Feature sprint — scaffold done, building user-facing features.  
...
2. ~~`quote-preview` — display labor / material / VAT / margin / total~~ ✅ done (commit `edd086f`)
...
- [x] 0.1 Confirm Bun is installed: `bun --version` (must be ≥ 1.0)
- [x] 0.2 Confirm PocketBase binary exists or download it to `infra/pocketbase/`
- [x] 0.3 `bun init` → create `package.json`
- [x] 0.4 Install runtime deps: `bun add next react react-dom pocketbase`
- [x] 0.5 Install dev deps: `bun add -d typescript @types/react @types/node vitest @vitejs/plugin-react tailwindcss @tailwindcss/vite`
- [x] 0.6 Install shadcn/ui: `bunx shadcn@latest init` (choose defaults)
- [x] 0.7 Create `tsconfig.json`, `next.config.ts`, `vitest.config.ts`, `tailwind.config.ts`
- [x] 0.8 Install OpenSpec globally: `bun add -g @fission-ai/openspec@latest && openspec init`
- [x] 0.9 Add Linear design system: `npx getdesign@latest add linear.app` (creates `DESIGN.md` in root — reference for all UI work)
- [x] 0.10 Verify `CLAUDE.md` exists and contains the gstack section (already created — just confirm)
- [x] 0.11 Verify `AGENTS.md` contains the tool-routing table (already updated — just confirm)
- [x] **CHECKPOINT 0**: `git add -A && git commit -m "chore: environment setup" && git push origin main`

### Phase 1 — Domain Layer (pure logic, no framework, ~20 min)

- [x] 1.1 Write `src/domain/types.ts`
...
- [x] 1.2 Write `src/domain/calculators.ts`
...
- [x] 1.3 Write `tests/domain/calculators.test.ts`
  - Test labor total against sample takeoff (expected: 12×0.5 + 18×0.25 + 8×0.25 + 4×0.35 + 1×4.0 + 150×0.02 + 200×0.02 = 24.9 hours × 85 = 2116.50)
...
- [x] **RUN**: `bun test tests/domain/` — must pass 100%
- [x] **CHECKPOINT 1**: `git add -A && git commit -m "feat: domain types and calculators" && git push origin main`

### Phase 2 — I/O Layer (~20 min)

- [x] 2.1 Write `src/io/load-takeoff.ts` — reads and validates takeoff JSON, returns `TakeoffItem[]` + settings
- [x] 2.2 Write `src/io/load-catalog.ts` — parses catalog CSV, returns `Map<sku, Product>`
- [x] 2.3 Write `src/io/load-kits.ts` — reads kits JSON, returns `Kit[]`
- [x] 2.4 Write `src/io/export-csv.ts`
  - `exportQuoteCsv(quote)` → CSV string (sku, name, supplier, qty, unit price, total)
  - `exportSupplierCsv(group)` → CSV string per supplier
- [x] 2.5 Write `src/io/pocketbase-admin.ts` — PocketBase JS SDK admin client, export `pb` instance
- [x] 2.6 Write `tests/io/loaders.test.ts` — parse sample files, assert shapes and counts

- [x] **RUN**: `bun test tests/io/` — must pass 100%
- [x] **CHECKPOINT 2**: `git add -A && git commit -m "feat: io layer — loaders and csv export" && git push origin main`

### Phase 3 — CLI Smoke Test (~10 min)

- [x] 3.1 Write `src/index.ts` — run full pipeline on sample data, print quote summary to terminal
- [x] 3.2 **RUN**: `bun run src/index.ts`
  - Expected output includes: labor total ≈ $2,116.50, material breakdown, CEBO/Rexel split
  - Verify numbers by hand against sample data before continuing

- [x] **CHECKPOINT 3**: `git add -A && git commit -m "feat: cli smoke test passes" && git push origin main`

### Phase 4 — API Route (~15 min)

- [x] 4.1 Write `src/app/api/quote/route.ts`
...
- [x] 4.2 Write `tests/api/quote.test.ts` — POST sample takeoff, assert correct totals

- [x] **RUN**: `bun test tests/api/` — must pass 100%
- [x] **CHECKPOINT 4**: `git add -A && git commit -m "feat: quote API route" && git push origin main`
...
- [x] 5.1 Start PocketBase in background: `./infra/pocketbase/pocketbase serve &`
- [x] 5.2 Wait for PocketBase to be ready: `sleep 3 && curl -s http://localhost:8090/api/health`
- [x] 5.3 Create admin account via API (only works on first run, no auth needed):
  ```bash
  ...
  ```
- [x] 5.4 Write `infra/pocketbase/pb_migrations/` JS migration files to create all collections from `docs/pocketbase-schema.md` (PocketBase auto-applies these on startup):
...
- [x] 5.5 Restart PocketBase to apply migrations: kill the background process, re-run `./infra/pocketbase/pocketbase serve &`
- [x] 5.6 Write `scripts/seed-pocketbase.ts` — seeds products + kits from sample CSV/JSON via PocketBase JS SDK
- [x] 5.7 **RUN**: `bun run scripts/seed-pocketbase.ts`
- [x] 5.8 Verify via API (no UI needed):
  ```bash
...
- [x] **CHECKPOINT 5**: `git add -A && git commit -m "feat: pocketbase collections + seed script" && git push origin main`

### Phase 6 — Minimal UI Shell (~15 min)

- [x] 6.1 Write `src/app/layout.tsx` — root layout, Tailwind globals, Inter font. Follow `DESIGN.md` (Linear style: dark background `#0F0F0F`, Inter font, purple accent `#5E6AD2`, tight spacing)
- [x] 6.2 Write `src/app/page.tsx` — single-page prototype:
...
- [x] **RUN**: `bun dev` — Next.js starts without errors, page loads and shows quote

- [x] **CHECKPOINT 6**: `git add -A && git commit -m "feat: minimal ui shell — pipeline visible end to end" && git push origin main`

### Phase 7 — Final Verification (~10 min)

- [x] 7.1 `bun test` — full suite, 0 failures
- [x] 7.2 `bun run src/index.ts` — correct numbers
- [x] 7.3 `bun dev` — UI loads, quote displays
- [x] 7.4 PocketBase running, seed data present
- [x] 7.5 All checkpoints committed

- [x] **CHECKPOINT 7 (DONE)**: `git add -A && git commit -m "chore: prototype scaffold complete" && git push origin main`
...
1. **Takeoff entry form** — user can enter items manually (no PDF parsing) ← **NEXT**
2. ~~**Quote preview** — live-calculated display with breakdown~~ ✅ done (commit `edd086f`)
